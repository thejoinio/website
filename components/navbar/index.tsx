  ...
  return (
    <header className="fixed top-0 z-50 w-full font-[family-name:var(--font-tsb)] border-b border-[#374151] bg-[#09141B]/80 backdrop-blur">
      <nav className="flex justify-between items-center w-full max-w-[1440px] mx-auto h-20 px-4 md:px-10 lg:px-20">
        ...
        <ul className={`${isNavOpened? "translate-x-0": "-translate-x-full"} md:translate-x-0 transition duration-200 flex flex-col md:flex-row items-center gap-14 md:gap-10 lg:gap-[60px] fixed md:relative top-0 left-0 pt-28 md:pt-0 w-full md:w-auto h-screen md:h-fit bg-black md:bg-transparent`}>
        ...
        <button onClick={()=>setIsNavOpened(!isNavOpened)} className="flex md:hidden hover:scale-90 active:scale-90 transition duration-200 p-2.5 pr-0 z-50">{isNavOpened? <CloseMenu /> :<JamMenu />}</button>
      </nav>
    </header>
  );
};

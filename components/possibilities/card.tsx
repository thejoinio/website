...
export const Card: React.FC<{heading?: string, image: string|StaticImageData, width: number, height: number, details?: string}> = ({heading="Means of Exchange", image, width, height, details="Use JOIN as a secure and transparent means of exchange within the ecosystem."}) => {
    ...
    return (
        <div onBlur={()=>setActive(false)} onClick={()=>setActive(!active)} className="flex flex-col items-center relative group w-full max-w-[272px] aspect-square transition-all duration-1000 gap-2.5 hover:gap-5 border border-white/80 rounded-[14px] p-5 bg-opacity-100 bg-[linear-gradient(98deg,rgba(102,254,203,0.20)_6.1%,rgba(137,64,255,0.20)_103.66%)] z-10">
            ...
            <div className="flex max-h-[160px] w-fit justify-center items-center p-6">
                <Image src={image} alt="#" width={width} height={height} className={`max-w-[${width}] w-fit h-auto mx-auto transition-all duration-1000 scale-90 group-hover:scale-110 object-scale-down`} />
            </div>
